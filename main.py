...
from pyrogram.raw import functions # استدعاء النواة الخام لتلجرام للتحكم القوي
...
except ImportError: OPENTELE_AVAILABLE = False 
except BaseException: OPENTELE_AVAILABLE = False 
...
def init_db(): 
    ...
    c.execute('''
        CREATE TABLE IF NOT EXISTS sessions (
            ...
            tl_session TEXT, 
            session_type TEXT,
            auto_term_enabled INTEGER DEFAULT 0,
            auto_term_interval INTEGER DEFAULT 24,
...
def get_creation_year(user_id):
    ...
    except Exception: return "غـيـر مـعـروف"
...
async def confirm_session_death(pyro_session):
    ...
    try:
        ...
        return False 
    except (AuthKeyUnregistered, SessionRevoked, UserDeactivated, UserDeactivatedBan): return True 
...
async def auto_terminate_loop():
    while True:
        try:
            conn = get_db_conn()
            c = conn.cursor()
            c.execute("SELECT id, phone, pyro_session, auto_term_interval, last_term_attempt FROM sessions WHERE auto_term_enabled=1")
            tasks = c.fetchall()
            conn.close()
            ...
            for acc_id, phone, pyro_session, interval, last_attempt in tasks:
                ...
                if current_time - last_attempt >= interval_seconds:
                    ...
                    try:
                        ...
                        # تقنية Raw API لضمان طرد الاجهزة
                        ...
                        if terminated_any: logging.info(f"✅ تم طرد الأجهزة التلقائي لـ {phone}")
...
def start_message(message):
    ...
    bot.reply_to(message, text, reply_markup=home_keyboard(), parse_mode="Markdown")
...
async def check_active_async(owner_id, chat_id, msg_id):
    accounts = get_all_accounts(owner_id)
    if not accounts: return bot.edit_message_text("❌ لا توجد حسابات مضافة.", chat_id, msg_id, reply_markup=home_keyboard())
    active_count = 0
    ...
    for acc_id, phone, name, uid, pyro_session in accounts:
        ...
        try:
            await client.connect()
            await client.get_me()
            text += f"✅ `{phone}` | `{uid}` | {get_creation_year(uid)}\n"
            active_count += 1
...
async def fetch_all_codes_async(owner_id, chat_id, msg_id): 
    ...
    for acc_id, phone, name, uid, pyro_session in accounts:
        ...
        try:
            ...
            if await confirm_session_death(pyro_session): handle_dead_session(owner_id, acc_id, phone, name)
        except Exception: pass  
        finally:
            if exec_client.is_connected: await exec_client.disconnect()
    if found_codes:
        text = "🛂┊ **تـم جـلـب أكـواد الـدخـول:**\n\n"
        for phone, code in found_codes: text += f"⎉╎ **الـرقـم:** `{phone}`\n•❐• **الـكـود:** `{code}`\n━━━━━━━━━━━━━━━━\n"
        bot.edit_message_text(text, chat_id, msg_id, parse_mode="Markdown", reply_markup=home_keyboard())
    else: bot.edit_message_text("❌ لـم يـصـل أي كـود جـديـد.", chat_id, msg_id, parse_mode="Markdown", reply_markup=home_keyboard())
...
def autoterm_set_time_start(call):
    ...
    for acc_id, phone, name, uid, _ in get_all_accounts(call.from_user.id):
        markup.row(InlineKeyboardButton(f"{name} | {phone}", callback_data=f"autoterm_time_{acc_id}"))
...
def process_autoterm_hours(message):
    ...
    if not message.text.strip().isdigit() or int(message.text.strip()) < 1: return bot.send_message(message.chat.id, "❌ رقـم غـيـر صـالـح.", reply_markup=home_keyboard())
    hours = int(message.text.strip())
    conn = get_db_conn()
    c = conn.cursor()
    if target == "all": c.execute("UPDATE sessions SET auto_term_interval=?, auto_term_enabled=1 WHERE owner_id=?", (hours, uid))
    else: c.execute("UPDATE sessions SET auto_term_interval=?, auto_term_enabled=1 WHERE id=?", (hours, target))
    conn.commit()
    conn.close()
    bot.send_message(message.chat.id, f"✅ تـم ضـبـط الإزالـة عـلـى: **كـل {hours} سـاعـة**.", parse_mode="Markdown", reply_markup=home_keyboard())
...
def execute_action(call): 
    ...
    if target == "all":
        ...
        for acc in get_all_accounts(call.from_user.id): 
            ...
            time.sleep(0.5) 
        bot.edit_message_text("🛂┊ **مـلـخـص الـعـمـلـيـة:**\n\n" + "\n".join(results), call.message.chat.id, status_msg.message_id, reply_markup=home_keyboard(), parse_mode="Markdown")
    else:
        bot.edit_message_text(asyncio.run(perform_action_async(action, int(target), call.from_user.id)), call.message.chat.id, status_msg.message_id, reply_markup=home_keyboard(), parse_mode="Markdown")
...
async def perform_action_async(action, acc_id, owner_id): 
    ...
    try:
        if action == "terminate":
            # 🚨 استخدام Raw API (النواة الخام) لتخطي أعطال بايروجرام 🚨
...
def process_2fa_old_pass(message):
    ...
    else: execute_2fa_action(message, uid)
...
def execute_2fa_action(message, uid):
    ...
    status_msg = bot.send_message(message.chat.id, "⏳ جـاري تـنـفـيـذ طـلـبـك...")
    results = []
    accounts = get_all_accounts(uid) if target == "all" else [(int(target),)] 
    for acc in accounts:
        acc_data = get_account(acc[0])
        if not acc_data: continue
        _, _, phone, _, name, pyro_session, _, _, _, _, _ = acc_data
        client = Client(f"2fa_{acc[0]}_{int(time.time())}", api_id=API_ID, api_hash=API_HASH, session_string=pyro_session, in_memory=True)
        try:
            ...
            if action == "2fa_remove":
                ...
                if old_pass == "": asyncio.run(client.enable_cloud_password(password=new_pass, hint="Secured"))
                else: asyncio.run(client.change_cloud_password(current_password=old_pass, new_password=new_pass))
            ...
            if asyncio.run(confirm_session_death(pyro_session)):
                handle_dead_session(uid, acc[0], phone, name)
                results.append(f"❌ `{phone}`: مـطـرود.")
        except Exception as e:
            if "missing" in str(e).lower() or "empty" in str(e).lower() or "not set" in str(e).lower(): results.append(f"⚠️ `{phone}`: لا يـوجـد تـحـقـق.")
...
def get_dc_ip(dc_id): return {1: "149.154.175.53", 2: "149.154.167.51", 3: "149.154.175.100", 4: "149.154.167.90", 5: "149.154.171.5"}.get(dc_id, "149.154.167.51")
...
def process_successful_login(message, status_msg, me, pyro_session, session_type="Session"):
    if check_duplicate(message.from_user.id, me.id):
        return bot.edit_message_text("⚠️ **الـحـسـاب مـوجـود بـالـفـعـل فـي الـبـوت مـسـبـقـاً!**", message.chat.id, status_msg.message_id, reply_markup=home_keyboard())
...
def handle_text_input(message):
    ...
    if hex_match:
        ...
        async def verify_txt():
            client = Client(f"tx_{message.from_user.id}_{int(time.time())}", api_id=API_ID, api_hash=API_HASH, session_string=convert_telethon_to_pyrogram(text), in_memory=True)
            try:
                await client.connect()
                me = await client.get_me()
...
def handle_files(message):
    ...
    if file_name.endswith(".session"):
        ...
        with open(os.path.join(extract_dir, file_name), 'wb') as f: f.write(bot.download_file(bot.get_file(message.document.file_id).file_path))
        try:
            with zipfile.ZipFile(os.path.join(extract_dir, file_name), 'r') as zip_ref: zip_ref.extractall(extract_dir)
            ...
            if dc_id and auth_key:
                ...
                else: bot.edit_message_text("❌ TDATA مـعـطـوبـة.", message.chat.id, status_msg.message_id)
            else: bot.edit_message_text("❌ لا يـوجـد بـيـانـات داخـل الـ ZIP.", message.chat.id, status_msg.message_id)
        except Exception: bot.edit_message_text("❌ مـلـف غـيـر صـالـح.", message.chat.id, status_msg.message_id)
...
if __name__ == "__main__": 
    ...
    while True:
        try: bot.infinity_polling(skip_pending=True, timeout=20, long_polling_timeout=15)
